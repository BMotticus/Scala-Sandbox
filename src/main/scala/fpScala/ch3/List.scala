...
//TODO: Remember, An ADT is just a data type defined by 1 or more _data constructors_, each of which may contain 0 or more arguements.
...
sealed trait List[+A] //TODO: Note: `List` data type, `parameterized` on a type, `A`
case object Nil extends List[Nothing] //TODO: Note: a `List` _data constructor_ representing the empty list
/* Another data constructor, representing nonempty lists. Note that `tail` is another `List[A]`,
which may be `Nil` or another `Cons`.
 */
case class Cons[+A](head: A, tail: List[A]) extends List[A] //TODO: Note: a `List` _data constructor_ representing the non-empty list
...
object ex1 extends App {
  println(s"The value of x = ${List.x}")
