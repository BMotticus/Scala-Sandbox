 ...
 *    
 * We can explicitly pattern match on an Option, we’ll almost always use the 
...
sealed trait Option[+A] { // Option data type, `parameterized` on a type, `A`
  /*
    ...
    the syntax:   obj.fn(arg1) or obj fn arg1 
    instead of:   fn(obj, arg1).
...
/** 
  ...
  *  A common pattern is to transform an Option via calls to map, flatMap, and/or filter, 
  *  and then use getOrElse to do error handling at the end
  */
...
/**
  ...
  */
  ...
  def filterViaMatch(f: A => Boolean): Option[A] = this match {
    ...
    case _ => None // Use the wildcard to return None when this is defined but the predicate `f(v)` yields false
  }

}

case class Some[+A](get: A) extends Option[A] // a Option _data constructor_ representing a defined type A
case object None extends Option[Nothing]     // a Option _data constructor_ representing an undefined type 
...
object Option {
  ...
  */
  ...
  def sequenceViaTraverse[A, B](a: List[Option[A]]): Option[List[A]] = traverse(a)(x => x) //Use identity function
