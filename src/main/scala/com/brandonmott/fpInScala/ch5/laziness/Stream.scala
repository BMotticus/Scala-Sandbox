package com.brandonmott.fpInScala.ch5.laziness

sealed trait Stream[+A] {
import Stream._
  /**
    * headOption must "explicitly" `force` the h `thunk` via `h()`.
    * In this example, we do NOT evaluate the tail of the Cons.
    */
  ...
  /**
    * toList is a helper function for inspecting streams 
  ...
  /**
    ...
    * drop on the invoked tail of a Cons cell. 
    * Note that the implementation is also tail recursive.
    */
...
/** @INCREMENTAL_function_implementation */
/*
@Note that these `map`, `filter`, `append`, and `flatMap` implementations are `INCREMENTAL`.
— they don’t fully generate their answers.
It’s not until some other computation looks at the elements of the resulting Stream that the computation to generate that Stream actually takes place—and then it will do just enough work to generate the requested elements.
...
*/
  ...
  zipAll(that)                                    // transforms into Stream[(Option[A],Option[B])] 
    //The Option indicates whether each stream has been exhausted.
...
object Stream {
  ...
  /** @Infinite_streams_and_corecursion */
  ...
  /**
    ...
    * @param f  function with type signature:  S => Option[(A, S)]
    *       Takes the initial state `S` and produces  either  a 
  ...
  Corecursive functions need not terminate so long as they remain "productive". (Which just means that we can always evaluate more of the result in a finite amount of time.)
